...
from config import (
    DEFAULT_MODEL,
    DATA_DIR,
    WEEKLY_DIGEST_GENRE_INTERVAL,
    WEEKLY_DIGEST_RETRY_ATTEMPTS,
    WEEKLY_DIGEST_RETRY_INTERVAL,
    WEEKLY_DIGEST_TEST_MODE,
    WEEKLY_DIGEST_TEST_GENRE_INTERVAL,
    WEEKLY_DIGEST_TEST_RETRY_INTERVAL,
)
...
class WeeklyDigestGenerator:
    ...
    def generate_weekly_digest(self, days_back: int = 7) -> Optional[str]:
        ...
        try:
            ...
            weekly_digest_content = self._create_weekly_digest_narrative(
                genre_summaries, document_groups
            )
            ...
            digest_path = self._save_digest(
                weekly_digest_content, days_back, document_groups
            )
    ...
    def _create_genre_summaries(
        self, document_groups: Dict[str, List[Document]]
    ) -> Dict[str, str]:
        ...
        print(
            f"⏱️  Rate limiting: {genre_interval / 60:.1f} minutes between genres, {WEEKLY_DIGEST_RETRY_ATTEMPTS} retries at {retry_interval / 60:.1f} minute intervals"
        )
        ...
        for i, (genre, documents) in enumerate(document_groups.items()):
            ...
            genre_summary = self._create_single_genre_summary_with_retries(
                genre, documents
            )
            ...
            if i < len(genre_list) - 1:
                print(
                    f"⏳ Waiting {genre_interval / 60:.1f} minutes before next genre..."
                )
    ...
    def _create_single_genre_summary_with_retries(
        self, genre: str, documents: List[Document]
    ) -> Optional[str]:
        """Create a comprehensive summary for a single genre with retry logic"""
        _, retry_interval = self._get_rate_limiting_intervals()

        for attempt in range(
            WEEKLY_DIGEST_RETRY_ATTEMPTS + 1
        ):  # +1 for initial attempt
            try:
                if attempt > 0:
                    print(
                        f"🔄 {genre} retry attempt {attempt}/{WEEKLY_DIGEST_RETRY_ATTEMPTS}"
                    )
                ...
                if summary:
                    ...
                    print(
                        f"⚠️  {genre} attempt {attempt + 1} failed: No summary generated"
                    )
    ...
    def _create_single_genre_summary(
        self, genre: str, documents: List[Document]
    ) -> Optional[str]:
...
"""
        ...
        try:
            ...
            if (
                response and len(response.strip()) > 50
    ...
    def _create_weekly_digest_narrative(
        self,
        genre_summaries: Dict[str, str],
        ...
        for attempt in range(
            ...
            try:
                if attempt > 0:
                    print(
                        f"🔄 Weekly digest retry attempt {attempt}/{WEEKLY_DIGEST_RETRY_ATTEMPTS}"
                    )

                # Try to create the digest narrative
                narrative = self._create_digest_narrative_content(
                    genre_summaries, document_groups
                )
                ...
                if narrative:
                    ...
                    print(
                        f"⚠️  Weekly digest attempt {attempt + 1} failed: No narrative generated"
                    )
    ...
    def _create_digest_narrative_content(
        self,
        genre_summaries: Dict[str, str],
        document_groups: Dict[str, List[Document]],
    ) -> Optional[str]:
...
"""
        ...
        if (
            response and len(response.strip()) > 200
        ):  # Ensure we got a substantial response
            return response.strip()
        else:
    ...
    def _save_digest(
        ...
        digest_content: str,
        days_back: int,
        document_groups: Dict[str, List[Document]] = None,
    ) -> Optional[str]:
        """Save the digest to a markdown file and structured JSON data"""
        ...
        try:
            ...
            json_filename = f"weekly_digest_{timestamp}.json"

            # Full path
            digest_dir = os.path.join(DATA_DIR, "digests")
            file_path = os.path.join(digest_dir, filename)
            json_path = os.path.join(digest_dir, json_filename)
...
"""
            ...
            # Save structured JSON data for Notion publishing
            if document_groups:
                json_data = self._create_json_metadata(document_groups, days_back)
    ...
    def _create_json_metadata(
        self, document_groups: Dict[str, List[Document]], days_back: int
    ) -> Dict:
        """Create structured metadata for Notion publishing"""
        end_date = datetime.now()
        ...
        for genre, documents in document_groups.items():
            newsletters_data = []
            for doc in documents[:5]:  # Limit to 5 per genre
            ...
            total_newsletters += len(documents)
    ...
    def get_recent_digests(self, limit: int = 5) -> List[str]:
        ...
        digest_files.sort(
            key=lambda x: os.path.getctime(os.path.join(digest_dir, x)), reverse=True
        )
